...
if ~isnan(mc)
    ...
    try
        ...
        if contains(ME.message, 'Auditory Toolbox')
            warning('supdeq_interpHRTF:AudioToolboxMissing', 'Magnitude Correction requires Auditory Toolbox which is not available. Skipping MC postprocessing.');
            mcEnabled = false;
        else
            rethrow(ME);
        end
    ...
    if ~mcEnabled
        % If Auditory Toolbox is not available, skip MC and return without MC
        % Output struct will be written below without MC-related fields
    else
